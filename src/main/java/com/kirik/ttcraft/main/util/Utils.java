...
public class Utils {
	...
	public static Location unserializeLocation(String str) {
		...
		return new Location(Bukkit.getWorld(split[5]), Double.valueOf(split[0]), Double.valueOf(split[1]),
				Double.valueOf(split[2]), Float.valueOf(split[3]), Float.valueOf(split[4]));
	}
	...
	public static <T> List<Class<? extends T>> getSubClasses(Class<T> baseClass) {
		...
		if (file.isDirectory() || (file.isFile() && !file.getName().endsWith(".jar"))) {
			...
			try {
				jarFile = new JarFile(file); // TODO close
			} catch (IOException e) {
				e.printStackTrace();
				return ret;
			}
	 ...
	 * 
	 * Pattern pathPattern =
	 * Pattern.compile(packageName.replace('.','/')+"/(.+\\.class)");
	 * final Enumeration<JarEntry> entries = jarFile.entries();
	 * while (entries.hasMoreElements()) {
	 * Matcher matcher = pathPattern.matcher(entries.nextElement().getName());
	 * if (!matcher.matches())
